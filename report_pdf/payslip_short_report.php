...
//My header codes
    ...
    $this->Cell(70,10,iconv( 'UTF-8','TIS-620','ชื่อ - สกุล'),1,0,'C'); 
    $this->Cell(45,10,iconv( 'UTF-8','TIS-620','ลายมือชื่อ'),1,0,'C'); 
    $this->Cell(45,10,iconv( 'UTF-8','TIS-620','หมายเหตุ'),1,0,'C'); 
    $this->Ln();
}  
}
...
function Empl_Salaly_detail(){
    ...
    global $BankCode;
     ...
     $sum_income = $Salary + $OVT10 + $OVT15 + $OVT20 + $OVT30 + $PosiAllow;
     $sum_expense = $SocTaxAmt;
     $final_total = $sum_income - $sum_expense;

    $this->SetFont('angsa','',18);
    $this->Ln(2);
    $this->Cell(0,10,iconv( 'UTF-8','TIS-620','ใบจ่ายเงินเดือน' ),0,1,"C");
    $this->Cell(0,10,iconv( 'UTF-8','TIS-620',$Owner_T_Name ),0,1,"C");
    $this->SetFont('angsa','',14);
    $this->SetY(0);
    $this->Ln(35);
    ...
    $this->Ln(8);
	$w=array(38,15,15,25,5,38,15,15,25);
    $this->SetFont('angsa','',15);  
    $this->SetTextColor(0,0,0); 
    $this->Ln(3);
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ชื่อรายการ'),1,0,'C'); 
    $this->Cell($w[1],10,iconv( 'UTF-8','TIS-620','จำนวน'),1,0,'C'); 
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620','หน่วย'),1,0,'C'); 
    $this->Cell($w[3],10,iconv( 'UTF-8','TIS-620','จำนวนเงิน'),1,0,'C'); 
    $this->Cell($w[4],10,iconv( 'UTF-8','TIS-620',''),0,0,'L'); 
    $this->Cell($w[5],10,iconv( 'UTF-8','TIS-620','รายการหัก'),1,0,'C'); 
    $this->Cell($w[6],10,iconv( 'UTF-8','TIS-620','จำนวน'),1,0,'C'); 
    $this->Cell($w[7],10,iconv( 'UTF-8','TIS-620','หน่วย'),1,0,'C'); 
    $this->Cell($w[8],10,iconv( 'UTF-8','TIS-620','จำนวนเงิน'),1,0,'C'); 
    $this->Ln();
    $fill = false;
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ค่าจ้าง/เงินเดือน'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,$WkDay,'LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620','วัน'),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format($Salary,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5],10,iconv( 'UTF-8','TIS-620','หักขาดงาน'),'LR',0,'L',$fill);
    $this->Cell($w[6],10,number_format($LevHrs,0),'LR',0,'C',$fill);
    $this->Cell($w[7],10,iconv( 'UTF-8','TIS-620','ชม.'),'LR',0,'C',$fill);
    $this->Cell($w[8],10,number_format($LevAmt,2),'LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ค่าล่วงเวลา 1 เท่า'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,$OVT10HR,'LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620','ชม.'),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format($OVT10,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5],10,iconv( 'UTF-8','TIS-620','หักลา'),'LR',0,'L',$fill);
    $this->Cell($w[6],10,number_format(0,0),'LR',0,'C',$fill);
    $this->Cell($w[7],10,iconv( 'UTF-8','TIS-620','ชม.'),'LR',0,'C',$fill);
    $this->Cell($w[8],10,number_format(0,2),'LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ค่าล่วงเวลา 1.5 เท่า'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,$OVT15HR,'LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620','ชม.'),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format($OVT15,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5],10,iconv( 'UTF-8','TIS-620','หักมาสาย'),'LR',0,'L',$fill);
    $this->Cell($w[6],10,number_format(0,0),'LR',0,'C',$fill);
    $this->Cell($w[7],10,iconv( 'UTF-8','TIS-620','ชม.'),'LR',0,'C',$fill);
    $this->Cell($w[8],10,number_format(0,2),'LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ค่าล่วงเวลา 2 เท่า'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,$OVT20HR,'LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620','ชม.'),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format($OVT20,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5],10,iconv( 'UTF-8','TIS-620','หักประกันสังคม'),'LR',0,'L',$fill);
    $this->Cell($w[6],10,'','LR',0,'C',$fill);
    $this->Cell($w[7],10,'','LR',0,'C',$fill);
    $this->Cell($w[8],10,number_format($SocTaxAmt,2),'LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ค่าล่วงเวลา 3 เท่า'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,$OVT30HR,'LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620','ชม.'),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format($OVT30,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5],10,iconv( 'UTF-8','TIS-620',''),'LR',0,'L',$fill);
    $this->Cell($w[6],10,'','LR',0,'C',$fill);
    $this->Cell($w[7],10,iconv( 'UTF-8','TIS-620','.'),'LR',0,'C',$fill);
    $this->Cell($w[8],10,'','LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','เงินประจำตำแหน่ง'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,'','LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620',''),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format($PosiAllow,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5],10,iconv( 'UTF-8','TIS-620',''),'LR',0,'L',$fill);
    $this->Cell($w[6],10,'','LR',0,'C',$fill);
    $this->Cell($w[7],10,iconv( 'UTF-8','TIS-620','.'),'LR',0,'C',$fill);
    $this->Cell($w[8],10,'','LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ค่าโทรศัพท์'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,'','LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620',''),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format(0,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5],10,iconv( 'UTF-8','TIS-620',''),'LR',0,'L',$fill);
    $this->Cell($w[6],10,'','LR',0,'C',$fill);
    $this->Cell($w[7],10,iconv( 'UTF-8','TIS-620','.'),'LR',0,'C',$fill);
    $this->Cell($w[8],10,'','LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','เบี้ยขยัน'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,'','LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620',''),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format(0,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    ...
    $this->Cell($w[8],10,'','LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ค่าอาหาร'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,'','LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620',''),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format(0,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    ...
    $this->Cell($w[8],10,'','LR',0,'R',$fill);
    $this->Ln();
    $this->Cell($w[0],10,iconv( 'UTF-8','TIS-620','ค่าครองชีพ'),'LR',0,'L',$fill);
    $this->Cell($w[1],10,'','LR',0,'C',$fill);
    $this->Cell($w[2],10,iconv( 'UTF-8','TIS-620',''),'LR',0,'C',$fill);
    $this->Cell($w[3],10,number_format(0,2),'LR',0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    ...
    $this->Cell($w[0] + $w[1] + $w[2],10,iconv( 'UTF-8','TIS-620','รวมรายได้'),1,0,'L',$fill);
    $this->Cell($w[3],10,number_format($sum_income,2),1,0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5] + $w[6] + $w[7],10,iconv( 'UTF-8','TIS-620','รวมรายการหัก'),1,0,'L',$fill);
    $this->Cell($w[8],10,number_format($sum_expense,2),1,0,'R',$fill);
    $this->Ln();

    $this->Cell($w[0] + $w[1] + $w[2],10,iconv( 'UTF-8','TIS-620','เงินรับสุทธิ'),1,0,'L',$fill);
    $this->Cell($w[3],10,number_format($final_total,2),1,0,'R',$fill);
    $this->Cell($w[4],10,'','LR',0,'L',$fill);
    $this->Cell($w[5] + $w[6] + $w[7] +$w[8] ,10,iconv( 'UTF-8','TIS-620',''),1,0,'L',$fill);
    $this->Ln();
...
$file_name = "payroll_short_payslip_period".$period."_term".$term."_".$todate."";
